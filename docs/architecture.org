...
- =mult? edit [char] ESC= do =edit= as many times as specified by =mult?=
- =visual [text_object|motion]= change visual selection mode and alter selection

** State Machine
One way to translate typed commands into functions is through a state machine.

| State\Input | mult0  | multN | verb | modifier | text_object | motion | search_char | edit | undo | repeat | ASCII |
|-------------+--------+-------+------+----------+-------------+--------+-------------+------+------+--------+-------|
| mult0       | multN  |       |      |          |             |        |             |      |      |        |       |
| multN       |        |       |      |          |             |        |             |      |      |        |       |
| verb        | multN  |       |      |          |             |        |             |      |      |        |       |
